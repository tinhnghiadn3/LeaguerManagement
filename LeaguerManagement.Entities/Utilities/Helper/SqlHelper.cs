...
namespace LeaguerManagement.Entities.Utilities.Helper
{
	public static class SqlHelper
	{
		...
		public class SprocResults
		{
			...
			public IList<T> ReadNextListOrEmpty<T>()
			{
		...
		/// <returns></returns>
		public static void ExecuteStoredProc(this DbCommand command, Action<SprocResults> handleResults, CommandBehavior commandBehaviour = CommandBehavior.Default, bool manageConnection = false)
		{
		...
		/// <summary>
		/// Executes a DbDataReader asynchronously/>.
		/// </summary>
		public static async Task ExecuteStoredProcAsync(this DbCommand command, Action<SprocResults> handleResults, CommandBehavior commandBehaviour = CommandBehavior.Default, CancellationToken ct = default(CancellationToken), bool manageConnection = false)
		{
			if (handleResults == null)
			{
				throw new ArgumentNullException(nameof(handleResults));
